'''
Author: souldream
Date: 2022-04-15 15:12:02
LastEditTime: 2022-05-06 02:44:46
LastEditors: souldream
Description: 
FilePath: \朵朵\qt\collection.py
...
class Collection:
    def __init__(self, user_inf, music):
        super().__init__()
        # 从文件中加载UI定义
        self.ui = uic.loadUi(r"ui\collection.ui")
        self.inf = user_inf
        self.column = 4
        self.row = 0
        ...
        # # 设置表头
        # self.ui.table_result.setHorizontalHeaderLabels(["","","歌曲", "歌手"])
    ...
    def showMusicList(self):
        ...
        self.ui.table_result.setColumnWidth(2, 430)
        self.ui.table_result.setColumnWidth(3, 430)
    ...
    def broad(self, index):
        ...
        search_music = self.ui.edit_search.text().strip()
        # 实例化另外一个窗口
        main_win = Broadcast(self.inf, self.cur_path, music, lrc, photo, 0, search_music)
        # 阻塞父窗口
        main_win.ui.setWindowModality(QtCore.Qt.ApplicationModal)
        # 显示新窗口
        main_win.ui.show()
    ...
    def check(self, index):
        ...
        # path = './temp/root/'
        # # 暂存地址
        # copy_path = './temp/root/'+  self.song_list[idx].strip() + ' - ' + self.music_list[idx].strip() + '.mp3'
        # 收藏地址
        music_path = self.cur_path + '/' + self.song_list[idx].strip() + ' - ' + self.music_list[idx].strip() + '.mp3'
        ...
        # else:
        #     try:
        #         shutil.copyfile(copy_path, music_path)
    ...
    def set_button2(self, index):
        ...
        if(index):
            ...
            # btn.clicked.connect(lambda:self.comboxSelect('rrrrrrrrrrrr'))
            self.ui.table_result.setCellWidget(index, 2, self.check_list[f'music2{index}'])
            ...
            # btn.clicked.connect(lambda:self.comboxSelect('rrrrrrrrrrrr'))
            self.ui.table_result.setCellWidget(index, 3, self.check_list[f'song2{index}'])
        else:
            data = ["歌曲", "歌手"]
            heart = QtWidgets.QPushButton()
            heart.setStyleSheet("QPushButton{border-image: url(img/base/search_background.png);background-color: rgb(246, 227, 178);}")
