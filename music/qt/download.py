'''
Author: souldream
Date: 2022-04-15 20:40:35
LastEditTime: 2022-05-06 02:44:35
LastEditors: souldream
Description: 
FilePath: \朵朵\qt\download.py
...
class Download:
    def __init__(self, user_inf, music):
        super().__init__()
        # 从文件中加载UI定义
        self.ui = uic.loadUi(r"ui\download.ui")
        self.inf = user_inf
        self.column = 5
        self.row = 0
...
#################################################################################
    def broad(self, index):
        ...
        search_music = self.ui.edit_search.text().strip()
        # 实例化另外一个窗口
        main_win = Broadcast(self.inf, self.cur_path, music, lrc, photo, 1, search_music)
        # 阻塞父窗口
        main_win.ui.setWindowModality(QtCore.Qt.ApplicationModal)
        # 显示新窗口
        main_win.ui.show()
    ...
    def broad2(self, index):
        ...
        search_music = self.ui.edit_search.text().strip()
        # 实例化另外一个窗口
        main_win = Broadcast(self.inf, self.cur_path, music, lrc, 1, search_music)
        # 阻塞父窗口
        main_win.ui.setWindowModality(QtCore.Qt.ApplicationModal)
        # 显示新窗口
        main_win.ui.show()
